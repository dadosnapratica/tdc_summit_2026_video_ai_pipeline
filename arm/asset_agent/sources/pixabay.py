...
def search(query: str, cfg: SearchConfig) -> List[Dict[str, Any]]:
    ...
    proxy_token = (os.getenv("WORKSHOP_ACCESS_TOKEN") or "").strip().strip('"')
    ...
    try:
        r = get_with_retry(
            ...
            params=(
                {"q": query, "per_page": cfg.per_source, "safesearch": "true"}
                if use_proxy
                else {"key": key, "q": query, "per_page": cfg.per_source, "safesearch": "true"}
            ),
            ...
            out.append(
                {
                    ...
                    "author": it.get("user", ""),
                    "tags": it.get("tags", "") or "",
                    "attribution_url": it.get("pageURL", ""),
                    "query": query,
        ...
        logger.warning("[pixabay] image search falhou: %s", e)
    ...
    try:
            ...
            out.append(
                {
                    ...
                    # Pixabay não devolve thumbnail pronto por URL; picture_id é o id do thumb.
        ...
        logger.warning("[pixabay] video search falhou: %s", e)
