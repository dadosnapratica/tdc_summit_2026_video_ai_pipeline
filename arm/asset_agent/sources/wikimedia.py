...
def search(query: str, cfg: SearchConfig) -> List[Dict[str, Any]]:
    ...
    try:
        # 1) search por arquivos no Commons
        r = requests.get(
        ...
        # 2) imageinfo para obter URLs + licença
        r = requests.get(
            "https://commons.wikimedia.org/w/api.php",
            params={
                "action": "query",
        ...
        for _, page in pages.items():
            ...
            image_desc = safe_get(meta, ["ImageDescription", "value"], "")
            ...
            out.append(
                {
                    ...
                    "text": re.sub(r"<.*?>", "", str(image_desc)) if image_desc else "",
                    "attribution_url": page_url or url,
                    "query": query,
                }
            )
    except Exception as e:
        logger.warning("[wikimedia] search falhou: %s", e)
