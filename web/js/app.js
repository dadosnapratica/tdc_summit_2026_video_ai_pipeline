...
function wireResearchUseRoteiro(btn, payload) {
  btn.onclick = () => {
    const tema = (payload.tema || $("research_kw").value || "").trim();
...
function wireTrendTemaOnly(btn, tema) {
  btn.onclick = () => {
    $("tema").value = (tema || $("research_kw").value || "").trim();
    $("angulo").value = "";
    youtubeContext = null;
    setActiveTab("roteiro");
  };
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (healthBtn) {
    healthBtn.onclick = () => window.open("/health", "_blank", "noopener,noreferrer");
  }
  ...
  $("btn-angles").onclick = async () => {
    try {
      ...
      if (!tema) {
        alert("Informe o tema antes de pedir sugestões de ângulo.");
        return;
      }
